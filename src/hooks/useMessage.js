...
export const useMessage = () => {
    ...
    const setActiveEvent = ( messagesConductor ) => {
        dispatch( onSetActiveEvent( messagesConductor ) );
    }

    //* Funcion para grabar un nuevo evento en el modal
    const startSavingEvrnt = async( messagesConductor ) => {

       try {

          if ( messagesConductor.id ) {
            // estoy Actualizando el event si tiene el mismo id...
            await calendarApi.put(`/events/${ messagesConductor.id }`, messagesConductor );
            dispatch( onUpdateEvent({ ...messagesConductor, user }) );
            return;

          } 
            // creando uno nuevo evento...
            const { data } = await calendarApi.post('/events', messagesConductor);
            dispatch( onAddNewEvent({ ...messagesConductor, id: data.evento.id, user  }) );
